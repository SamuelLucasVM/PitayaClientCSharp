...
//        }
        ...
        {
            StaticPitayaBindingCS.PcUnityLibInit((int)_currentLogLevel, null, null, OnAssert, "mac", "1", "2.2.4");
        ...
        public static PcClient CreateClient(bool enableTls, bool enablePolling, bool enableReconnect, int connTimeout, IPitayaListener listener)
        {
            ...
            var handlerId = StaticPitayaBindingCS.PcClientAddEvHandler(client, (PcEventCallbackDelegate)NativeEventCallback, IntPtr.Zero, null);
        ...
        //{
        //    string certPath = Path.Combine(GetStreamingAssetsPath(), name);

        //    if (OperatingSystem.IsAndroid())
        //    {
        //        var persistentPath = Path.Combine(GetPersistentDataPath(), name);
        ...
        //    string streamingAssetsPath = Path.Combine(executablePath, "StreamingAssets");
        ...
        private static void OnAssert(string e, string file, int line)
        {
            //Debug.LogAssertion(string.Format("{0}:{1} Failed assertion {2}", file, line, e));
        }
